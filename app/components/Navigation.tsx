'use client';

import Image from 'next/image';
import Link from 'next/link';
import { useState } from 'react';

export default function Navigation() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  return (
    <nav className="bg-moon-teal text-white sticky top-0 z-40 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          {/* Logo */}
          <div className="flex items-center space-x-3">
            <Image
              src="/images/Moon House Logo.jpg"
              alt="Moon House Logo"
              width={60}
              height={60}
              className="rounded-full"
            />
            <div>
              <h1 className="text-2xl font-bold">Moon House</h1>
              <p className="text-sm text-moon-yellow">Coffee, Games & Grub</p>
            </div>
          </div>

          {/* Desktop Navigation */}
          <div className="hidden md:flex space-x-8">
            <a href="#home" className="hover:text-moon-yellow transition">
              Home
            </a>
            <a href="#gallery" className="hover:text-moon-yellow transition">
              Gallery
            </a>
            <a href="#order" className="hover:text-moon-yellow transition">
              Order
            </a>
            <a href="#rewards" className="hover:text-moon-yellow transition">
              Rewards
            </a>
            <a href="#contact" className="hover:text-moon-yellow transition">
              Contact
            </a>
          </div>

          {/* Mobile menu button */}
          <button
            className="md:hidden text-white"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
          >
            <svg
              className="h-6 w-6"
              fill="none"
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth="2"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              {isMenuOpen ? (
                <path d="M6 18L18 6M6 6l12 12" />
              ) : (
                <path d="M4 6h16M4 12h16M4 18h16" />
              )}
            </svg>
          </button>
        </div>
      </div>

      {/* Mobile Navigation */}
      {isMenuOpen && (
        <div className="md:hidden bg-moon-teal-dark">
          <div className="px-2 pt-2 pb-3 space-y-1">
            <a
              href="#home"
              className="block px-3 py-2 hover:bg-moon-teal-light rounded"
              onClick={() => setIsMenuOpen(false)}
            >
              Home
            </a>
            <a
              href="#gallery"
              className="block px-3 py-2 hover:bg-moon-teal-light rounded"
              onClick={() => setIsMenuOpen(false)}
            >
              Gallery
            </a>
            <a
              href="#order"
              className="block px-3 py-2 hover:bg-moon-teal-light rounded"
              onClick={() => setIsMenuOpen(false)}
            >
              Order
            </a>
            <a
              href="#rewards"
              className="block px-3 py-2 hover:bg-moon-teal-light rounded"
              onClick={() => setIsMenuOpen(false)}
            >
              Rewards
            </a>
            <a
              href="#contact"
              className="block px-3 py-2 hover:bg-moon-teal-light rounded"
              onClick={() => setIsMenuOpen(false)}
            >
              Contact
            </a>
          </div>
        </div>
      )}
    </nav>
  );
}
